...
fileprivate var twitterAccount: ACAccount?
...
open class TwitterRequest: NSObject
{
    ...
    private func performTwitterRequest(_ method: SLRequestMethod, handler: @escaping (PropertyList?) -> Void) {
        ...
        let request = SLRequest(
            ...
            url: URL(string: "\(TwitterAPI.Constants.urlPrefix)\(self.requestType)\(jsonExtension)"),
            parameters: self.parameters
        )
        performTwitterSLRequest(request!, handler: handler)
    }
    
    private func performTwitterSLRequest(_ request: SLRequest, handler: @escaping (PropertyList?) -> Void) {
            ...
            accountStore.requestAccessToAccounts(with: twitterAccountType, options: nil) { (granted, _) in
                if granted {
                    if let account = accountStore.accounts(with: twitterAccountType)?.last as? ACAccount {
                        ...
                        let error = "No Twitter account?"
                        self.log(error as AnyObject)
                        handler(error as TwitterRequest.PropertyList?)
                    }
                } else {
                    let error = "Twitter access denied"
                    self.log(error as AnyObject)
                    handler(error as TwitterRequest.PropertyList?)
                }
            }
